...
def translate_actinide(inp_dict, names, mode_nuclide):
    ...
    for k, v in inp_dict.items():
        ...
        mat = None#openmc.Material(material_id = 100*k[0] + k[1] + 1,name=str(k))
        mat.set_density('sum')
...
def make_model():
    ...
    m_arg = '/mnt/e/temp/reactions.xml'
    n_arg = '/mnt/e/temp/InOut6800.dat.hdf5'
    ...
    print("FINISH!!!")
    return diction_1, nuclide, diction_2, element, diction_3, densna, dzz, hss, rods, type_fuel, type_assembly
...
def make_model_mg_by_nuclide():
    ...
    materials_list = openmc_geometry.get_all_materials()
    materials_file = openmc.Materials([v for v in materials_list.values()])
    materials_file.cross_sections = 'libxs.h5'
    materials_file.export_to_xml()
    # Instantiate a Settings object, set all runtime parameters, and export to XML
    settings_file = openmc.Settings()
    ##########################SETTINGS
